...
class BusDataset(data.Dataset):
    ...
    def __init__(self, bus, train, window):
        if train:
        ...
        # Extract cmf targets from DataFrame.
    ...
    def __getitem__(self, item):
        # print(item)
        try:
            inputs = torch.tensor(self.df.iloc[item:item+self.window].values).type(torch.FloatTensor)
            pos_targets = torch.tensor(self.cmf['pos'].iloc[item:item+self.window].values).type(torch.FloatTensor)
            neg_targets = torch.tensor(self.cmf['neg'].iloc[item:item+self.window].values).type(torch.FloatTensor)
...
def train(args):
    ...
    # Load data
    train_bus_data = BusDataset('proov_00*', train=True, window=16)
    train_data_loader = DataLoader(train_bus_data, batch_size=args.batch_size, drop_last=True)
    test_bus_data = BusDataset('proov_00*', train=False, window=16)
    test_data_loader = DataLoader(test_bus_data, batch_size=len(test_bus_data), drop_last=True)
    ...
    for step, (batch_inputs, pos_targets, neg_targets) in enumerate(train_data_loader):
        ...
        p_loss = pos_criterion(torch.squeeze(p_out), pos_targets.view(args.batch_size, input_size))
        p_loss.backward()
        pos_optimizer.step()

        # import pdb; pdb.set_trace()
        n_loss = neg_criterion(torch.squeeze(n_out), neg_targets.view(args.batch_size, input_size))
        n_loss.backward()
        neg_optimizer.step()
    ...
    for step, (test_inputs, pos_targets, neg_targets) in enumerate(test_data_loader):
        ...
        p_test_loss = pos_criterion(torch.squeeze(p_test_out), pos_targets.view(pos_targets.shape[0], input_size))
        p_test_acc = accuracy(p_test_out, pos_targets, len(p_test_out[0]), args.acc_bound)
        n_test_loss = pos_criterion(torch.squeeze(n_test_out), neg_targets.view(neg_targets.shape[0], input_size))
